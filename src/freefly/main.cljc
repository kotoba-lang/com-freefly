(ns freefly.main
  "Kotodama WASM entrypoint for the Freefly clean-room actor (L4) — Clojure port.
  ...
  facts (`freefly.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "freefly")
(def tier "L4")
...
(def entity-specs
  [{:entity "Drone"      :plural "drones"      :id-prefix "freefly_dro"
   ...
   {:entity "Telemetry"  :plural "telemetries" :id-prefix "freefly_tel"
   ...
   {:entity "Payload"    :plural "payloads"    :id-prefix "freefly_pay"
    :fields [:droneId :type :weightKg]                     :required [:type :weightKg]
    :coerce {:weightKg :float}              :refs {:droneId "Drone"}}
   {:entity "Swarm"      :plural "swarms"      :id-prefix "freefly_swa"
...
(defn emit-facts
  "EAVT facts for one record: {\"freefly.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "freefly-compat" :tier tier :entities entities} 200])
...
(defn start! [] :freefly-compat/ready)
